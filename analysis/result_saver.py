...
def save(stats: dict, ecn_params: Optional[list] = None,
         ...
         rate_samples: Optional[list] = None,
         seg_samples_list: Optional[list] = None) -> Tuple[str, bool]:
    ...
    if segment_diffs:
        ...
        status = f"PASS:{seg_pass} FAIL:{seg_fail}"
    else:
    ...
    if segment_diffs:
        for d in segment_diffs:
            verdict = "FAIL" if d["over_threshold"] else "PASS"
            lines.append(
                f"  Seg {d['seg']:>2}: {d['max_diff']:>6.2f}%"
                f" ({d['flow0_rate']:>6.2f} Gbps/{d['flow0_pct']:>5.2f}%,"
                f" {d['flow1_rate']:>6.2f} Gbps/{d['flow1_pct']:>5.2f}%)"
                f" -{verdict}"
            )
    else:
        lines.extend([
    ...
    # Save per-segment CSVs
    if seg_samples_list:
        for seg_idx, seg_samples in enumerate(seg_samples_list, 1):
            csv_file = output_dir / f"segment_{seg_idx}.csv"
            with open(csv_file, "w", newline="", encoding="utf-8") as f:
                ...
                for row in seg_samples:
                    writer.writerow(row)
            log.info(f"CSV saved: {csv_file} ({len(seg_samples)} points)")
    elif rate_samples:
    ...
    return str(summary_file), not stopped_early
